...
class OrderDetailDialog extends ConsumerWidget {
  ...
  // --- FUNGSI UNTUK GENERATE DAN TAMPILKAN PDF ---
  Future<void> _printPdf(FullOrder order) async {
      ...
      build: (pw.Context context) {
        return pw.Column(
          ...
          children: [
            ...
            pw.Text('Nama: ${order.customerName}'),
            pw.Text('Alamat: ${order.customerDetails?['address'] ?? 'N/A'}'),
            pw.Text('WhatsApp: ${order.customerDetails?['whatsapp'] ?? 'N/A'}'),
            pw.Divider(height: 30, thickness: 2),
             ...
             pw.Row(
               ...
               children: [
                 pw.Column(
                   ...
                   children: [
                      pw.Text('Subtotal Produk: ${_formatCurrency(order.subtotal)}'),
                      pw.Text('Biaya Pengiriman: ${_formatCurrency(order.shippingFee)}'),
                      pw.SizedBox(height: 10),
                      pw.Text('Total: ${_formatCurrency(order.total)}', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
                   ]
                 )
               ]
             )
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final orderData = ref.watch(singleOrderProvider(orderId));
    ...
    return AlertDialog(
      ...
      content: SizedBox(
        ...
        child: orderData.when(
          ...
          error: (err, stack) => const Center(child: Text('Gagal memuat detail pesanan.')),
          data: (order) {
            if (order == null) {
            ...
            return SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  ListTile(title: const Text('Pelanggan'), subtitle: Text(order.customerName)),
                  ...
                  ...order.products.map((p) => ListTile(
                        leading: CircleAvatar(
                          ...
                          child: p.image.isEmpty ? const Icon(Ionicons.cube_outline) : null,
                        ),
                        title: Text(p.name),
                  ...
                  const Divider(),
                  ListTile(title: const Text('Subtotal'), trailing: Text(_formatCurrency(order.subtotal))),
                  ...
                  ListTile(
                    ...
                    trailing: Text(_formatCurrency(order.total), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                  ),
                ],
              ),
            );
