...
import '../../widgets/reports/receivable_list.dart'; // Widget ini akan kita buat selanjutnya
...
class _ReceivableReportScreenState extends State<ReceivableReportScreen> {
  ...
  Future<void> _generateReport() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Gagal menghasilkan laporan piutang: $e')),
      );
    } finally {
      setState(() {
        _isLoading = false;
  ...
  Widget _buildReportContent() {
    ...
    if (_reportData!.isEmpty) {
      return const Center(
        child: Text('Tidak ada piutang untuk rentang tanggal yang dipilih.'),
      );
    }

    return ReceivableList(reportData: _reportData!);
  }
}
